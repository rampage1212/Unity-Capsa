...
public class Straight : IEvaluator<Straight> {
	...
	protected override void PreEvaluate () {
		PreEvaluateStraight (cardSet[0], ref bucket, ref special);
	}
	...
	public static void PreEvaluateStraight(Card firtCard, ref CardSet bucket, ref CardSet special) {
		bucket.Clear ();
		bucket.Add (firtCard);
		
		special.Clear ();
		if (firtCard.Nominal == "3")
			special.Add (firtCard);
	}
